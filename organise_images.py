...
for (dirpath, dirnames, filenames) in walk(folder_path):
    ...
    for single_file in filenames:
        ...
        if file_extension.lower() == '.heic':
            ...
            original_time = time.gmtime(os.stat(complete_path).st_birthtime)
            creation_year = original_time.tm_year
            creation_month = original_time.tm_mon

            if not(creation_year in assigned_heic_years):
                create_year_directory(str(creation_year), heic_path)
                assigned_heic_years.append(creation_year)

            file_name = os.path.basename(file_name)
            new_name = heic_path + '/' + str(creation_year) + '/' + str(creation_year) + '_' + str(creation_month).zfill(2) + '/' + file_name + file_extension
            ...
            if new_name in assigned_heic_names:
                ...
                while repeated_name:
                    repeated_counter += 1
                    new_name = heic_path + '/' + str(creation_year) + '/' + str(creation_year) + '_' + str(creation_month).zfill(2) + '/' + file_name + '_' + str(repeated_counter) + file_extension
